namespace ServiceInsight.Tests
{
    ...
    class SequenceDiagramModelCreatorTestsForJson4 : SequenceDiagramModelCreatorTestsFromJson
    {
        ...
        public void ShouldHaveHandlersProperlyOrdered()
        {
            ...
            Assert.AreEqual("UserSignupRequestAccepted", ModelCreator.Handlers[1].Name);
            Assert.AreEqual("CreateCRMAccountRequest", ModelCreator.Handlers[2].Name);
            Assert.AreEqual("CreateCRMAccountResponse", ModelCreator.Handlers[3].Name);
            Assert.AreEqual("CreateCRMContactRequest", ModelCreator.Handlers[4].Name);
            Assert.AreEqual("CreateCRMContactCreatedResponse", ModelCreator.Handlers[5].Name);
            Assert.AreEqual("LinkCRMAccountAndContactRequest", ModelCreator.Handlers[6].Name);
            Assert.AreEqual("LinkCRMAccountAndContactResponse", ModelCreator.Handlers[7].Name);
            Assert.AreEqual("LogCRMAudit", ModelCreator.Handlers[8].Name);
            Assert.AreEqual("NewAccountCreated", ModelCreator.Handlers[9].Name);
            Assert.AreEqual("SendAddAccountEmailRequestAccepted", ModelCreator.Handlers[10].Name);
            Assert.AreEqual("ADUserCreated", ModelCreator.Handlers[11].Name);
            Assert.AreEqual("StoreUserActivationKeyInCache", ModelCreator.Handlers[12].Name);
        }
    }
}